...
# a ='../simulations_data/REVISIONS_1/Sp_input_e4_'+str(ff_value)+'pA'+str(portion_e4)+'%_e5_'+str(fb_value_5)+'pA_'+str(portion_e5)+'%/'
a ='../simulations_data/REVISIONS_1/Distributed_weights_spontaneous_lognormal_std_equals_mean_trial_0/'
...
runtime = 1500.0 * ms # How long you want the simulation to be
...
def get_lognormal_params(mean_val, std_val):
    """
    Given a desired *linear* mean and std for a lognormal distribution,
    returns (mu, sigma) for the underlying Normal( mu, sigma^2 ).
    """
    var_val = std_val**2
    sigma_sq = np.log(1.0 + var_val / (mean_val**2))
    sigma_log = np.sqrt(sigma_sq)
    mu_log = np.log(mean_val) - 0.5*sigma_sq
...
# Now is passed in the function no need to have it here
    ...
    # CSV to write values of mean and std weights
    csv_file = open("../simulations_data/distributed_weights/connection_stats.csv", "w", newline='')
    writer = csv.writer(csv_file)
    writer.writerow(["source_pop", "target_pop", "original_weight", "emp_mean", "emp_std"])
    stdFrac = 1 # Fraction of the mean which will be the standard deviation of the lognormal distribution
    for h in range(len(sources)):
        print('Connecting source', h)
        for k in range(len(targets)):
            print('Connecting target', k)
            ...
            if s_cell_type == 0:
                ...
                s_cell_type_name = 'SST'
            elif s_cell_type == 3:
                s_cell_type_name = 'VIP'
            ...
            if t_cell_type == 0:
                ...
                t_cell_type_name = 'SST'
            elif t_cell_type == 3:
                t_cell_type_name = 'VIP'
            ...
            if s_cell_type==0: # sendind is excitatory neuron
                ...
                if t_cell_type==0: # target is excitatory neuron
                    ...
                    if Cp[4*s_layer+s_cell_type][4*t_layer+t_cell_type]==0: #If the probability of connections is 0 I still need to create an AMPA that has a weight with 0 effect
                        ...
                        orig_w_ampa = wp* G*Cs[4*s_layer+s_cell_type][4*t_layer+t_cell_type]/(e_ampa*Cp[4*s_layer+s_cell_type][4*t_layer+t_cell_type]*N[s_layer][s_cell_type])
                        mean_ampa = orig_w_ampa
                        std_ampa = stdFrac * mean_ampa
                        mu_lognorm_ampa, sigma_lognorm_ampa = get_lognormal_params(mean_ampa, std_ampa)
                        conn.w_AMPA = f'exp({mu_lognorm_ampa} + {sigma_lognorm_ampa} * randn())'
                        emp_mean = np.mean(conn.w_AMPA[:])
                        emp_std = np.std(conn.w_AMPA[:])
                        print('Original weight AMPA E-E:', orig_w_ampa)
                        print("Empirical mean of weights AMPA E-E:", emp_mean)
                        print("Empirical std of weights AMPA E-E: ", emp_std)
                        # CSV: source_pop, target_pop, mean, std
                        writer.writerow(
                            [f"L{s_layer}-type{s_cell_type}", 
                                f"L{t_layer}-type{t_cell_type}",
                                f"{orig_w_ampa}",
                                f"{emp_mean}",
                                f"{emp_std}"]
                        )
                        # Plotting and saving weights
                        w_ampa_values = conn.w_AMPA[:]
                        if len(w_ampa_values) > 0:
                            min_val = np.min(w_ampa_values)
                            max_val = np.max(w_ampa_values)
                            # Create 50 log-spaced bins
                            bins = np.logspace(np.log10(min_val), np.log10(max_val), 50)
                            plt.figure()
                            plt.hist(w_ampa_values, bins=bins)                                         
                            plt.xscale('log') # set x-axis to logarithmic scale
                            plt.title(f"Distribution of AMPA synaptic weights from L{s_layer_name}{s_cell_type_name} to L{t_layer_name}{t_cell_type_name}")
                            plt.xlabel("Synaptic Weight")
                            plt.ylabel("Count")

                            plt.savefig(
                                f"../simulations_data/distributed_weights/hist_AMPA_sL{s_layer}_sT{s_cell_type}_tL{t_layer}_tT{t_cell_type}.png"
                            )
                            plt.close()
                    conn.delay='d'
                    ...
                    if nmda_on==True: #I need to create the NMDA connections
                        ...
                        if Cp[4*s_layer+s_cell_type][4*t_layer+t_cell_type]==0: #If the probability of connections is 0 I still need to create an NMDA that has a weight with 0 effect
                            ...
                            orig_w_NMDA = wp* G* Cs[4*s_layer+s_cell_type][4*t_layer+t_cell_type]/(e_nmda*Cp[4*s_layer+s_cell_type][4*t_layer+t_cell_type]*N[s_layer][s_cell_type])
                            mean_nmda = orig_w_NMDA
                            std_nmda = stdFrac * mean_nmda
                            mu_lognorm_nmda, sigma_lognorm_nmda = get_lognormal_params(mean_nmda, std_nmda)
                            conn1.w_NMDA = f'exp({mu_lognorm_nmda} + {sigma_lognorm_nmda} * randn())'
                            emp_mean = np.mean(conn1.w_NMDA[:])
                            emp_std = np.std(conn1.w_NMDA[:])
                            print('Original weight NMDA E-E:', orig_w_NMDA)
                            print("Empirical mean of weights NMDA E-E:", emp_mean)
                            print("Empirical std of weights NMDA E-E: ", emp_std)
                            # CSV: source_pop, target_pop, mean, std
                            writer.writerow(
                                [f"L{s_layer}-type{s_cell_type}", 
                                    f"L{t_layer}-type{t_cell_type}",
                            ...
                            # Plotting and saving weights
                            w_nmda_values = conn1.w_NMDA[:]
                            if len(w_nmda_values) > 0:
                                min_val = np.min(w_nmda_values)
                                max_val = np.max(w_nmda_values)
                                # Create 50 log-spaced bins
                                bins = np.logspace(np.log10(min_val), np.log10(max_val), 50)
                                plt.figure()
                                plt.hist(w_nmda_values, bins=bins, color='orange')
                                plt.xscale('log') # set x-axis to logarithmic scale
                                plt.title(f"Distribution of NMDA synaptic weights from L{s_layer_name}{s_cell_type_name} to L{t_layer_name}{t_cell_type_name}")
                                plt.xlabel("Synaptic Weight")
                                plt.ylabel("Count")
                                plt.savefig(
                                    f"../simulations_data/distributed_weights/hist_NMDA_sL{s_layer}_sT{s_cell_type}_tL{t_layer}_tT{t_cell_type}.png"
                                )
                                plt.close()
                        conn1.delay='d'
                        All_C.append(conn1)
                ...
                if t_cell_type!=0: # target is inhibitory neuron
                    ...
                    if Cp[4*s_layer+s_cell_type][4*t_layer+t_cell_type]==0: #If the probability of connections is 0 I still need to create an AMPA that has a weight with 0 effect
                        ...
                        orig_w_ampa = wp*G*Cs[4*s_layer+s_cell_type][4*t_layer+t_cell_type]/(i_ampa*Cp[4*s_layer+s_cell_type][4*t_layer+t_cell_type]*N[s_layer][s_cell_type])
                        mean_ampa = orig_w_ampa
                        std_ampa = stdFrac * mean_ampa
                        mu_lognorm_ampa, sigma_lognorm_ampa = get_lognormal_params(mean_ampa, std_ampa)
                        conn.w_AMPA = f'exp({mu_lognorm_ampa} + {sigma_lognorm_ampa} * randn())'
                        emp_mean = np.mean(conn.w_AMPA[:])
                        emp_std = np.std(conn.w_AMPA[:])
                        print('Original weight AMPA E-I:', orig_w_ampa)
                        print("Empirical mean of weights AMPA E-I:", emp_mean)
                        print("Empirical std of weights AMPA E-I: ", emp_std)
                        # CSV: source_pop, target_pop, mean, std
                        writer.writerow(
                            [f"L{s_layer}-type{s_cell_type}", 
                                f"L{t_layer}-type{t_cell_type}",
                    ...
                    if nmda_on==True: #I need to create the NMDA connections
                        ...
                        if Cp[4*s_layer+s_cell_type][4*t_layer+t_cell_type]==0: #If the probability of connections is 0 I still need to create an AMPA that has a weight with 0 effect
                            ...
                            orig_w_NMDA = wp*G* Cs[4*s_layer+s_cell_type][4*t_layer+t_cell_type]/(i_nmda*Cp[4*s_layer+s_cell_type][4*t_layer+t_cell_type]*N[s_layer][s_cell_type])
                            mean_nmda = orig_w_NMDA
                            std_nmda = stdFrac * mean_nmda
                            mu_lognorm_nmda, sigma_lognorm_nmda = get_lognormal_params(mean_nmda, std_nmda)
                            conn1.w_NMDA = f'exp({mu_lognorm_nmda} + {sigma_lognorm_nmda} * randn())'
                            emp_mean = np.mean(conn1.w_NMDA[:])
                            emp_std = np.std(conn1.w_NMDA[:])
                            print('Original weight NMDA E-I:', orig_w_NMDA)
                            print("Empirical mean of weights NMDA E-I:", emp_mean)
                            print("Empirical std of weights NMDA E-I: ", emp_std)
                            # CSV: source_pop, target_pop, mean, std
                            writer.writerow(
                                [f"L{s_layer}-type{s_cell_type}", 
                                    f"L{t_layer}-type{t_cell_type}",
                ...
                if Cp[4*s_layer+s_cell_type][4*t_layer+t_cell_type]==0:  #If the probability of connections is 0 I still need to create an GABA that has a weight with 0 effect
                    ...
                    orig_w_GABA = wp*G* Cs[4*s_layer+s_cell_type][4*t_layer+t_cell_type]/(Cp[4*s_layer+s_cell_type][4*t_layer+t_cell_type]*N[s_layer][s_cell_type])
                    mean_gaba = orig_w_GABA
                    std_gaba = stdFrac * mean_gaba
                    mu_lognorm_gaba, sigma_lognorm_gaba = get_lognormal_params(mean_gaba, std_gaba)
                    conn2.w_GABA = f'exp({mu_lognorm_gaba} + {sigma_lognorm_gaba} * randn())'
                    emp_mean = np.mean(conn2.w_GABA[:])
                    emp_std = np.std(conn2.w_GABA[:])
                    print('Original weight GABA:', orig_w_GABA)
                    print("Empirical mean of weights GABA:", emp_mean)
                    print("Empirical std of weights GABA: ", emp_std)
                    # CSV: source_pop, target_pop, mean, std
                    writer.writerow(
                        [f"L{s_layer}-type{s_cell_type}", 
                            f"L{t_layer}-type{t_cell_type}",
                    ...
                    # Plotting and saving weights
                    w_gaba_values = conn2.w_GABA[:]
                    if len(w_gaba_values) > 0:
                        min_val = np.min(w_gaba_values)
                        max_val = np.max(w_gaba_values)
                        # Create 50 log-spaced bins
                        bins = np.logspace(np.log10(min_val), np.log10(max_val), 50)
                        plt.figure()
                        plt.hist(w_gaba_values, bins=bins, color='green')
                        plt.xscale('log') # set x-axis to logarithmic scale
                        plt.title(f"Distribution of GABA synaptic weights from L{s_layer_name}{s_cell_type_name} to L{t_layer_name}{t_cell_type_name}")
                        plt.xlabel("Synaptic Weight")
                        plt.ylabel("Count")
                        plt.savefig(
                            f"../simulations_data/distributed_weights/hist_GABA_sL{s_layer}_sT{s_cell_type}_tL{t_layer}_tT{t_cell_type}.png"
                        )
                        plt.close()
...
def connect_l1to_target(targets,Gl1,Csl1,Cpl1,Nl1,pops,popl1,d):
    stdFrac = 1 # Fraction for lognormal std
    All_C_l1=[]
    for k in range(len(targets)):
            t_layer = targets[k][0]
            ...
            if Csl1[4*t_layer+t_cell_type]==0 or Cpl1[4*t_layer+t_cell_type]==0:
                ...
                orig_w_GABA = Gl1* Csl1[4*t_layer+t_cell_type]/(Cpl1[4*t_layer+t_cell_type]*Nl1)
                mean_gaba = orig_w_GABA
                std_gaba = stdFrac * mean_gaba
                mu_lognorm_gaba, sigma_lognorm_gaba = get_lognormal_params(mean_gaba, std_gaba)
                conn2.w_GABA = f'exp({mu_lognorm_gaba} + {sigma_lognorm_gaba} * randn())'
                emp_mean = np.mean(conn2.w_GABA[:])
                emp_std = np.std(conn2.w_GABA[:])
                print('Original weight GABA L1:', orig_w_GABA)
                print("Empirical mean of weights GABA L1:", emp_mean)
                print("Empirical std of weights GABA L1: ", emp_std)
            conn2.delay='d'
...
def connect_l1_l1(Gl1,Cs_l1_l1,Cp_l1_l1,Nl1,popl1,d):
    stdFrac = 1 # Frac of original mean for lognormal std
    ...
    orig_w_GABA = Gl1* Cs_l1_l1/(Cp_l1_l1*Nl1)
    mean_gaba = orig_w_GABA
    std_gaba = stdFrac * mean_gaba
    mu_lognorm_gaba, sigma_lognorm_gaba = get_lognormal_params(mean_gaba, std_gaba)
    conn2.w_GABA = f'exp({mu_lognorm_gaba} + {sigma_lognorm_gaba} * randn())'
    ...
    for h in range(len(sources)):
        ...
        if s_cell_type==0: #0 is excitatory neuron
            ...
            if Cs_tol1[4*s_layer+s_cell_type]==0 or Cp_tol1[4*s_layer+s_cell_type]==0:
                ...
                orig_w_ampa=Gl1* Cs_tol1[4*s_layer+s_cell_type]/(i_ampa*Cp_tol1[4*s_layer+s_cell_type]*N[s_layer][s_cell_type])
                mean_ampa = orig_w_ampa
                std_ampa = stdFrac * mean_ampa
                mu_lognorm_ampa, sigma_lognorm_ampa = get_lognormal_params(mean_ampa, std_ampa)
                conn.w_AMPA = f'exp({mu_lognorm_ampa} + {sigma_lognorm_ampa} * randn())'
                emp_mean = np.mean(conn.w_AMPA[:])
                emp_std = np.std(conn.w_AMPA[:])
                print('Original weight AMPA L1:', orig_w_ampa)
                print("Empirical mean of weights AMPA L1:", emp_mean)
                print("Empirical std of weights AMPA L1: ", emp_std)
            conn.delay='d'
            All_C.append(conn)
            del conn
            ...
            if nmda_on==True:
                ...
                if Cs_tol1[4*s_layer+s_cell_type]==0 or Cp_tol1[4*s_layer+s_cell_type]==0:
                    ...
                    orig_w_NMDA=Gl1*  Cs_tol1[4*s_layer+s_cell_type]/(i_nmda*Cp_tol1[4*s_layer+s_cell_type]*N[s_layer][s_cell_type])
                    mean_nmda = orig_w_NMDA
                    std_nmda = stdFrac * mean_nmda
                    mu_lognorm_nmda, sigma_lognorm_nmda = get_lognormal_params(mean_nmda, std_nmda)
                    conn1.w_NMDA = f'exp({mu_lognorm_nmda} + {sigma_lognorm_nmda} * randn())'
                    emp_mean = np.mean(conn1.w_NMDA[:])
                    emp_std = np.std(conn1.w_NMDA[:])
                    print('Original weight NMDA E-I L1:', orig_w_NMDA)
                    print("Empirical mean of weights NMDA E-I L1:", emp_mean)
                    print("Empirical std of weights NMDA E-I L1: ", emp_std)
                conn1.delay='d'
                All_C.append(conn1)
                del conn1
        else:
            ...
            if Cs_tol1[4*s_layer+s_cell_type]==0 or Cp_tol1[4*s_layer+s_cell_type]==0:
                ...
                orig_w_GABA=Gl1* Cs_tol1[4*s_layer+s_cell_type]/(Cp_tol1[4*s_layer+s_cell_type]*N[s_layer][s_cell_type])
                mean_gaba = orig_w_GABA
                std_gaba = stdFrac * mean_gaba
                mu_lognorm_gaba, sigma_lognorm_gaba = get_lognormal_params(mean_gaba, std_gaba)
                conn2.w_GABA = f'exp({mu_lognorm_gaba} + {sigma_lognorm_gaba} * randn())'
