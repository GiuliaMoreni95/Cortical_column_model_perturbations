...
# Link to the paper: https://www.biorxiv.org/content/10.1101/2024.04.02.587673v1.full
...
#Important to note: 
#If you want to run the program only once and you don't need the perturbation analyis comment this if/else
if len(sys.argv) > 2:
    param1 = sys.argv[1]
    param2 = sys.argv[2]
...
#Important to note: 
#If you want to run the program only once and you don't need the perturbation analyis:
#Uncomment the values here and DO NOT use param1,2,3

folder_spikes= param1 #folder containing that perturbation simulation, e.g. perturbation to e2/3
#The cell that is being perturbed is the index LAYER, CELL_TYPE (all the cells are perturbed)
LAYER= 0#int(param2) #0
CELL_TYPE= 0# int(param3) #0

#FF input
ff_value = 0#int(param4) #30 #FF input
ff_value_pv = ff_value
#FB input 
fb_value_5= 0#int(param5) # 0 #FB input
fb_value_pv_5 =fb_value_5

fb_value_23 =0 #int(param6)
fb_value_pv_23= fb_value_23 

fb_value_6 = 0#int(param7)
...
#percentages of cells receiving the inputs
portion_e4= 25
portion_pv4= 5
portion_e5= 25
portion_pv5= 5
...
folder_state='Simulation_spont_NumberNeuron_'+folder_spikes
#Files where you want to save the output of the simulation
a='../simulations_data/REVISIONS_1/'+folder_state
...
Ntot= int(param2) #5000 #Total number of neurons in the simulation
...
#IMPORTANT
# Here is where I can say which input I want for the neurons and in which "state" I want to be for then the perturbation analysis
# For example in this case I am giving 30pA of input to a all E4 
#I am in a FF state (Fig. 1B)
...
# and I am reproducing the perturbation analysis in Fig. 2B.


#Here is the perturbation input: 
#I need to give the perturbation input to one population for each simulation. 
#I run the program multiple times with the code script_iterations_perturbation.sh and each run the "LAYER" and "CELL_TYPE" is a different value. 

#pops[LAYER][CELL_TYPE][:].I_ext=- perturb_value * pA 

# #LATER 4
# if LAYER==1 and CELL_TYPE==0:
#     pops[1][0][:int(N[1][0]/(100/portion_e4))].I_ext= - (perturb_value+ff_value) * pA
# if LAYER==1 and CELL_TYPE==1:
#     pops[1][1][:int(N[1][1]/(100/portion_pv4))].I_ext= - (perturb_value+ff_value_pv) * pA
    
# #LATER 5
# if LAYER==2 and CELL_TYPE==0:
#     pops[2][0][:int(N[2][0]/(100/portion_e5))].I_ext= - (perturb_value+fb_value_5) * pA

# if LAYER==2 and CELL_TYPE==1:
#     pops[2][1][:int(N[2][1]/(100/portion_pv5))].I_ext= - (perturb_value+fb_value_pv_5) * pA


# #LATER 2/3
# if LAYER==0 and CELL_TYPE==0:
#     pops[0][0][:int(N[0][0]/(100/portion_e23))].I_ext= - (perturb_value+fb_value_23) * pA

# if LAYER==0 and CELL_TYPE==1:
#     pops[0][1][:int(N[0][1]/(100/portion_pv23))].I_ext= - (perturb_value+fb_value_pv_23) * pA

# #LATER 6
# if LAYER==3 and CELL_TYPE==0:
#     pops[3][0][:int(N[3][0]/(100/portion_e6))].I_ext= - (perturb_value+fb_value_6) * pA

# if LAYER==3 and CELL_TYPE==1:
#     pops[3][1][:int(N[3][1]/(100/portion_pv6))].I_ext= - (perturb_value+fb_value_pv_6) * pA





#pops[0][0][:int(N[0][0])].I_ext1=-30 * pA # If I have a second input at a different time going to another population, I need this Iext1
